/* Javascript script for creating weighted tree and actions for various events   
Define the action how the data/tool tip acted and displayed
Initialize the weight values to the tree
Read the required columns from RDU input csv file
*/
...
var valueField = "All";
var valueFields = ["All","GROW_INFLOW","REDUCE_CLAIM_LOSSES","REDUCE_EXPENSES",
					"Leads_prioritization(UC1)","Cross-sell(UC7)","Retention(UC9)","Channel_Management(UC24)",
					"Claims_payout_management(UC14)","Risk_portfolio_optimization(UC29)","Subrogation_recovery_management(UC32)","Fraud_Detection(UC18)",
					"Claims_process_optimization(UC34)","Underwriting_process_optimization(UC37)","Sales_process_optimization(UC39)", "High", "Medium", "Low"]; //"Test","subtest"



var formatCurrency = function (d) { if (isNaN(d)) d = 0; return "Number of features: " + d3.format(",.0f")(d) ; };

function loadData() {

    d3.csv("data/input.csv", function (csv) {
...
function prepData(csv) {
    ...
    var nest = d3.nest()
        .key(function (d) {
            return d.Level1 + ',' + d.Comments1; //d.Level1
        })
        .key(function (d) {
            return d.Level2 + ',' + d.Comments2;
        })
        .key(function (d) {
            return d.Level3 + ',' + d.Comments3;
        })
        .entries(values);
...
function initialize() {
    ...
    viz.data(data)                                                      // Expects hierarchical array of objects.
        ...
        .key(function (d) { return d.id })                             // Unique key (Technical Key)
        .value(function (d) {
...
function displayLabel(temp){
	if(temp.key){
		return temp.key.split(',')[0];
	}
	else{
		return temp['Level' + temp.depth];
	}
}
...
var datatip='<div class="tooltip" style="width: 200px; background-opacity:.5">' +
...
function createDataTip(x,y,h1,h2,h3) {
    ...
    d3.select("body")
        ...
        .style("left", (x - 125) + "px")
        .style("opacity",0)
		//.style("fontize",10)  //add font size
        .html(html)
...
function onMouseOver(e,d,i) {
	...
	if(d.key){
		createDataTip(rect.left, (rect.top+viz.height() *.05), d.key.split(',')[0], formatCurrency(d["agg_" + valueField]),d.key.split(',')[1]);
	}
	else{
		createDataTip(rect.left, (rect.top+viz.height() *.05), d['Level' + d.depth], formatCurrency(d["agg_" + valueField]),d['Comments' + d.depth]);
	}
	//createDataTip(rect.left, (rect.top+viz.height() *.05), ((String(d.key).split(',')[0]) + (d['Level' + d.depth])), formatCurrency(d["agg_" + valueField]),String(d.key).split(',')[1]);
    //createDataTip(rect.left, (rect.top+viz.height() *.05), (d.key || (d['Level' + d.depth])), formatCurrency(d["agg_" + valueField]),String(d.key).split(',')[0]); //d.depth
